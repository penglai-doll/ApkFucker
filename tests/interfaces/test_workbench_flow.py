...
def test_main_window_exports_markdown_report_from_menu_action(tmp_path: Path) -> None:
    app = _app()
    sample_path = tmp_path / "sample.apk"
    sample_path.write_bytes(b"apk")
    output_root = tmp_path / "artifacts"
    ...
    report_label = window.results_summary.report_path_label.text()
    report_path = Path(report_label.removeprefix("Exported Report: "))
