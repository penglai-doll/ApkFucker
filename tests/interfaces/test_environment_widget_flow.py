...
def test_main_window_refreshes_environment_status(tmp_path: Path) -> None:
    ...
    assert "Frida Bootstrap: unavailable" in window.task_center.execution_presets_value.text()
    ...
    frida_bootstrap_index = window.script_plan.execution_mode_combo.findData("real_frida_bootstrap")
    ...
    assert not window.script_plan.execution_mode_combo.model().item(frida_bootstrap_index).isEnabled()
    ...
    assert "Frida Bootstrap: ready" in window.task_center.execution_presets_value.text()
    ...
    assert window.script_plan.execution_mode_combo.model().item(frida_bootstrap_index).isEnabled()
